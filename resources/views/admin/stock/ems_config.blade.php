...
<section class="content">
    <div class="container">
        <form method="post" action="{{ route('admin.stock.ems_config.save') }}" enctype="multipart/form-data">
            @csrf
            <!-- /.row -->
            <div class="row">
                <div class="col-md-12">
                                ...
                                <div class="form-group col-md-5">
                                    ...
                                    <select class="form-control select2" name="inventory_id" style="width: 100%;" id="inventory_id">
                                        ...
                                        @foreach($inventories as $key => $inventory)
                                            <option value="{{ $inventory->ems_id }}" @if(!empty($data) && ($data->inventory_id == $inventory->ems_id)) selected="selected" @endif> {{ $inventory->username }} </option>
                                        @endforeach
                                    </select>
                                </div>
                                <div class="form-group col-md-5">
